...
describe('executeRipgrepWithLimit error branches', () => {
  it('returns partial output with warning when byte limit is hit mid-stream', async () => {
    ...
    const { output, warning } = await executeRipgrepWithLimit(['match', bigFile], 10);
  ...
  it('handles empty output gracefully', async () => {
    ...
    const { output, warning } = await executeRipgrepWithLimit(['zzz-nonexistent-pattern', noMatchFile], 1024);
    expect(output).toBe('');
